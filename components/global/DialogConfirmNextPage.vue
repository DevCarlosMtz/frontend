  ...
  <v-dialog v-model="dialog" :max-width="options.width" :style="{ zIndex: options.zIndex }">
    <v-card>
      ...
      <v-card-actions class="justify-end">
        <v-btn color="grey darken-2" text @click.native="cancel()">NO</v-btn>
        <v-btn ref="agreeBtn" color="blue darken-2" text @click.native="agree()">SI
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
